...
$(document).ready(function() {
 	$('#religion_code').val('${staffDto.religion_code }').attr('selected','selected');
 	
 	// 체크박스 체크하기 name='name명' , 특정값을 줄때에는 :checkbox[value='값'] 이렇게 넣어준다.
 	// 체크박스 체크하기 버전 jquery 1.6 이상부터는 prop
 	$("input:checkbox[name='school_code']:checkbox[value='${staffDto.school_code }']").prop('checked', true);

//	전체선택 시 ↓
// 	$("input:checkbox[name='skill']").prop('checked', true);
// 	$('#form input[name=school_code]').prop('checked', true);
	
// 스킬리스트 받아오기  
 	<c:forEach items="${skillList }" var="skill" varStatus="vs">
// 		alert('<c:out value="${skill}"/>'); 	
// 선택했던 기술 체크박스에 체크  ↓	
 		$("input:checkbox[name='skill']:checkbox[value='${skill.skill_code }']").prop('checked', true); 		
 	</c:forEach>
});
	...
	<table class="table_erp">
		...
		<tr>
			...
			<td>
				...
				<input type="text" id="staff_name" name="staff_name" value="${staffDto.staff_name }">
			</td>
			
			<th>주민번호</th>			
			<td>
				<input type="text" name="jumin_no" id="jumin_first" value="${jumin_first }">-
				<input type="password" name="jumin_no" id="jumin_second" value="${jumin_second }">
			</td>
		...
		<tr>
			<td colspan="6">
				...
				<a onclick="deleteStaff('${staffDto.staff_no}')" href="#"><input type="button" id="deleteBtn" value="삭제"></a>			
			</td>
		</tr>
		</tfoot>				
	</table>
...
$("#updateBtn").click(function(){
	...
	if (sch_check && skill_check) {		
		if (confirm("정말 수정 하시겠습니까?") == true){
			$("#form").attr({"target" : "_self","action":"updateStaff.do"}).submit();
		}else{
			return;
		}				
	}	
...
//삭제 버튼 클릭시 실행되는 function
function deleteStaff(seq){
	if (confirm("정말 삭제하시겠습니까??") == true){
		location.href='deleteStaff.do?seq='+seq;
	}else{
		return;
	}	
}
